...
def test_get_runtime():
    ...
    dirs = runs_dir
    timing_m = tools.get_runtime_all(
        id_filter="", dirs=dirs, all_times=False, levels=None, remove=None, verbose=True
    )
    timing = tools.get_runtime_all(
        id_filter="", dirs=dirs, all_times=True, remove=None, verbose=True
    )
...
def test_grid_combinations():
    param_grid = odict(
        input_sounding=["stable", "unstable"], res={"dx": [200, 4000], "dz0": [10, 50]}
    )
    ...
    param_combs_corr = pd.DataFrame(
        columns=["input_sounding", "dx", "dz0", "res_idx"], index=np.arange(6)
    )
    param_combs_corr.loc[:, :] = np.array(
        [
            ["stable", 200, 10, 0],
            ["stable", 4000, 50, 1],
            ["unstable", 200, 10, 0],
