...
def test_get_runtime():
    ...
    dirs = runs_dir
    timing_m = misc_tools.get_runtime_all(id_filter="", dirs=dirs, all_times=False,
                                          levels=None, remove=None, verbose=True)
    timing = misc_tools.get_runtime_all(id_filter="", dirs=dirs, all_times=True,
                                        remove=None, verbose=True)
    assert len(timing) == 345600
    assert timing.iloc[:, -7:].notna().all().all()

    with Capturing() as output:
        identical_runs = misc_tools.get_identical_runs(runs_dir + "WRF_pytest_eta_0",
                                                       "test_data/runs/")
    print(output)
    assert len(output) == 2
